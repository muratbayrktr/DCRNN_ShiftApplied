...
class DCRNNSupervisor(object):
    ...
    def __init__(self, adj_mx, **kwargs):
        ...
        if self._kwargs['model'].get('horizon') == 12:
            ...
            minute = "30min"
        name_str = f"Shifted_{dataset}_{options[self._kwargs.get('use_dtm')]}_{loss_name}_{self._kwargs['train'].get('base_lr')}_{self._kwargs.get('DTM_TH')}_{minute}_{date_of_run}"
    ...
    def run_epoch_generator(self, sess, model, data_generator, return_output=False, training=False, writer=None):
        ...
        # Peak Error
        max_value = tf.reduce_max(labels)
        feat_mean_values = tf.reduce_mean(labels,axis=0)
        pmae, pmape, prmse = metrics.calculate_peak_metrics_tf(pred=preds, label=labels, null_val=0, feat_mean_values=feat_mean_values,max_value=max_value,DTM_TH=self._DTM_TH)
        ...
        for _, (x, y) in enumerate(data_generator):
            ...
            pmaes.append(vals['pmae'])
            pmapes.append(vals['pmape'])
            prmses.append(vals['prmse'])
        ...
        results = {
            ...
            'mae': np.mean(maes),
            'pmae': np.mean(pmaes),
            'pmape': np.mean(pmapes),
            'prmse': np.mean(prmses)
        }
        if return_output:
            results['outputs'] = outputs
        end_time = time.time()
        #print('\tStep[{}]: train_mae: {:.4f}, loss: {:.4f}, {:.1f}s'.format(
        #        fetches['global_step'], results['mae'], results['loss'],(end_time-start_time) ))
        return results
    ...
    def _train(self, sess, base_lr, epoch, steps, patience=50, epochs=100,
        ...
        while self._epoch <= epochs:
            ...
            val_pmae, val_pmape, val_prmse = np.asscalar(val_results['pmae']), np.asscalar(val_results['pmape']), np.asscalar(val_results['prmse'])
    ...
    def evaluate(self, sess, **kwargs):
        ...
        for horizon_i in range(self._data['y_test'].shape[1]):
            ...
            pmae, pmape, prmse = metrics.calculate_peak_metrics(pred=y_pred,label=y_truths,max_value=max_value,feat_mean_values_np=feat_mean_values_np,null_val=0,DTM_TH=self._DTM_TH)
            ...
            self._logger.info(
                "Horizon {:02d}, MAE: {:.2f}, MAPE: {:.4f}, RMSE: {:.2f}, PMAE: {:.2f}, PMAPE: {:.2f}, PRMSE: {:.2f}".format(
                    horizon_i + 1, mae, mape, rmse, pmae, pmape, prmse
                )
            )
            
            utils.add_simple_summary(self._writer,
                                     ['%s_%d' % (item, horizon_i + 1) for item in
                                      ['metrics/pmae','metrics/pmape','metrics/prmse','metric/rmse', 'metric/mape', 'metric/mae']],
                                     [pmae, pmape, prmse, rmse, mape, mae],
                                     global_step=global_step)
        wandb.log({
            "TEST_MAE": np.mean(tmae),
            "TEST_MAPE": np.mean(tmape),
            "TEST_RMSE": np.mean(trmse),
            "PEAK_RMSE": np.mean(tprmse),
            "PEAK_MAE": np.mean(tpmae),
            "PEAK_MAPE": np.mean(tpmape),
        })
        outputs = {
